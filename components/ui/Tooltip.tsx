  ...
  return (
    <span className={`group/tooltip relative inline-flex ${className}`.trim()}>
      ...
      <span
        role="tooltip"
        aria-hidden="true"
        className={`pointer-events-none absolute left-1/2 z-30 -translate-x-1/2 ${position} whitespace-nowrap rounded-lg bg-[var(--gray-text)] px-2 py-1 text-[11px] font-bold text-[var(--bg-color)] opacity-0 shadow-sm transition-opacity duration-150 ease-out-expo group-hover/tooltip:opacity-100 group-focus-within/tooltip:opacity-100`}
      >
        {label}
      </span>
    </span>
