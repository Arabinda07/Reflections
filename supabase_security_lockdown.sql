...
create table if not exists public.ai_run_events (
  id uuid default gen_random_uuid() primary key,
  run_id uuid references public.ai_runs(id) on delete cascade not null,
  user_id uuid references auth.users(id) on delete cascade not null,
...
create index if not exists idx_ai_run_events_run_created_at
  on public.ai_run_events (run_id, created_at);

alter table public.ai_run_events enable row level security;

drop policy if exists "Users can view own AI run events" on public.ai_run_events;
create policy "Users can view own AI run events"
  on public.ai_run_events
  for select
  to authenticated
  using ((select auth.uid()) is not null and user_id = (select auth.uid()));

revoke all on table public.ai_run_events from anon, authenticated;
...
create table if not exists public.razorpay_subscriptions (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  plan_code text not null check (plan_code in ('weekly', 'monthly')),
